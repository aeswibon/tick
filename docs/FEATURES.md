...
2. For each required field (typed from Jira’s schema):
   ...
   - **User** (assignee, etc.) → cached list filtered in footer; **⌘R** (macOS) or **Ctrl+R** fetches more from Jira and **appends** to the cache (up to 500 per issue). Plain `r`/`R` type into the filter (e.g. names starting with R).
