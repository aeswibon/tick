...
```bash
tick issue show HIN-123 --site northwind
```
...
```bash
tick issue transition HIN-123 --to "In Progress" --site northwind
```
...
```bash
tick issue comment HIN-123 --body "Shipped in 1.0" --site northwind
echo "From CI" | tick issue comment HIN-123 --site northwind
```
...
```bash
tick issue assign HIN-123 --me --site northwind
tick issue assign HIN-123 --unassign --site northwind
```

## `tick issue watch` / `unwatch`

```bash
tick issue watch HIN-123 --site northwind
tick issue unwatch HIN-123 --site northwind
```
...
```bash
tick fields list --site northwind
tick fields list --site northwind --project HIN
```
...
```bash
tick search --jql 'project = HIN AND assignee = currentUser() ORDER BY updated DESC' --site northwind
```
...
```bash
tick search --jql 'assignee = currentUser() AND resolution = Unresolved' --site northwind \
  | jq -r '.issues[].key'
```
...
```bash
tick search --jql 'project = HIN AND status = Blocked' --site northwind \
...
```bash
tick search --jql 'updated >= -1d' --site northwind \
...
```bash
tick issue show HIN-42 --site northwind | jq '{ key, status, assignee, labels }'
```

Bulk result (after `tick bulk`):

```bash
tick bulk transition --site northwind --keys HIN-1,HIN-2 --to Done \
...
```bash
tick bulk transition --site northwind --keys HIN-1,HIN-2 --to Done
tick bulk assign --site northwind --keys HIN-1,HIN-2 --me
tick bulk labels --site northwind --keys HIN-1 --set "bug,triage"
```
...
```bash
0 18 * * 5   tick bulk transition --site northwind --keys "$(tick search --jql '...' --site northwind | jq -r '.issues[].key' | paste -sd,)" --to Done
```
...
- run: tick issue transition "$JIRA_KEY" --to "Done" --site northwind
  env:
