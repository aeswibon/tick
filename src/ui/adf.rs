    ...
    for node in content {
        match node.get("type").and_then(|t| t.as_str()) {
            Some("paragraph") => {
                ...
                if indent > 0 {
                    let mut indented_spans: Vec<Span<'static>> = vec![Span::raw(" ".repeat(indent * 2))];
            ...
            Some("heading") => {
                ...
                let mut spans: Vec<Span<'static>> = vec![
                    Span::styled(
                        ...
                        Style::default().fg(color_hex(0x89B4FA)).add_modifier(Modifier::BOLD),
                    ),
                ];
                if let Some(inline) = node.get("content").and_then(|c| c.as_array()) {
                    for child in inline {
                ...
                if let Some(items) = node.get("content").and_then(|c| c.as_array()) {
                    for item in items {
                        if let Some(item_content) = item.get("content").and_then(|c| c.as_array()) {
                            let rendered: Vec<Line<'static>> = render_content(item_content, indent + 1);
                            if let Some(first) = rendered.first() {
                                let mut spans: Vec<Span<'static>> = vec![Span::raw(format!("{}• ", "  ".repeat(indent)))];
                                spans.extend(first.spans.clone());
                                lines.push(Line::from(spans));
                                for rest in rendered.iter().skip(1) {
                                    let mut rest_spans: Vec<Span<'static>> = vec![Span::raw(format!("{}  ", "  ".repeat(indent)))];
                ...
                if let Some(items) = node.get("content").and_then(|c| c.as_array()) {
                    for (i, item) in items.iter().enumerate() {
                        if let Some(item_content) = item.get("content").and_then(|c| c.as_array()) {
                            let rendered: Vec<Line<'static>> = render_content(item_content, indent + 1);
                            if let Some(first) = rendered.first() {
                                let mut spans: Vec<Span<'static>> = vec![Span::raw(format!("{}{}. ", "  ".repeat(indent), i + 1))];
                                spans.extend(first.spans.clone());
                                lines.push(Line::from(spans));
                                for rest in rendered.iter().skip(1) {
                                    let mut rest_spans: Vec<Span<'static>> = vec![Span::raw(format!("{}  ", "  ".repeat(indent)))];
            ...
            Some("codeBlock") => {
                ...
                if !lang.is_empty() {
                    lines.push(Line::from(Span::styled(
                        format!("```{}", lang),
                        Style::default().fg(color_hex(0x66D9EF)).add_modifier(Modifier::BOLD),
                    )));
                } else {
                    lines.push(Line::from(Span::styled(
                        "```",
                    ...
                    for child in code_content {
                        if let Some(text) = child.get("text").and_then(|t| t.as_str()) {
                            for line in text.lines() {
                                lines.push(Line::from(Span::styled(
                                    line.to_string(),
                                    Style::default().fg(color_hex(0xE6DB74)).bg(color_hex(0x272822)),
                                )));
                            }
                        }
                    }
            ...
            Some("blockquote") => {
                if let Some(quote_content) = node.get("content").and_then(|c| c.as_array()) {
                    for quote_line in render_content(quote_content, indent) {
                        let mut spans: Vec<Span<'static>> = vec![
                            Span::styled("│ ", Style::default().fg(color_hex(0x45475A))),
                        ];
