...
pub fn draw_sprints(f: &mut Frame, app: &App, area: Rect) {
    let height = app.sprint_options.len() as u16 + 4;
    let popup = centered_rect(55, height.max(8), area);
    ...
    for (i, (_, name)) in app.sprint_options.iter().enumerate() {
        let selected = i == app.sprint_selected;
