...
**Same repo (tap):** This repository includes `Formula/tick.rb`. After the first release is published:
...
**Release attachment:** Each GitHub release also ships `tick.rb` (generated by CI). You can install from a release checkout with `brew install ./tick.rb`.

**Separate tap repo (optional):** Many projects eventually move the formula to a dedicated repo (e.g. `github.com/aeswibon/homebrew-tick`) so the main repo stays code-only. The release workflow can be extended to push updated SHAs there; for v0.1.0 the in-repo `Formula/` + release `tick.rb` are enough.
